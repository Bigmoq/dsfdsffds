...
export function AdminApplications() {
  ...
  const handleApplication = async (
    ...
    toast({
      ...
      description: approve ? "تمت الموافقة على الطلب بنجاح" : "تم رفض الطلب",
    });
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "pending": return <Badge variant="secondary" className="font-arabic">قيد المراجعة</Badge>;
      case "approved": return <Badge className="bg-green-500 font-arabic">مقبول</Badge>;
      case "rejected": return <Badge variant="destructive" className="font-arabic">مرفوض</Badge>;
      default: return null;
    }
  };
